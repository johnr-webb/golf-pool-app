...
router.post("/pools/:poolId/teams", requireAuth, async (req: AuthRequest, res) => {
  ...
  if (missingPlayers.length > 0) {
    res.status(400).json({ error: `Players not found: ${missingPlayers.join(", ")}` });
    return;
  }
...
router.put("/:teamId", requireAuth, async (req: AuthRequest, res) => {
  ...
  if (picks) {
    ...
    for (const doc of playerDocs) {
      ...
      if (doc.data()!.tournamentId !== pool.tournamentId) {
        res.status(400).json({ error: `Player ${doc.id} is not in this tournament` });
        return;
      }
    }
